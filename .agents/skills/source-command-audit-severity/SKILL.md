...
2. **Downgrade if mitigated.** A missing validation on a route only accepting localhost connections is MEDIUM, not HIGH. (Auralis is desktop-only and binds to localhost; see AGENTS.md.)
