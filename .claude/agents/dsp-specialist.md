...
- `auralis/core/simple_mastering.py` — SimpleMastering algorithm (actively modified — recent commits favor parallel processing to prevent spectral loss). Decomposed into `mastering_prepare.py`, `mastering_chunk_loop.py`, `mastering_process_chunk.py`, `mastering_diagnostics.py`, `mastering_notch_context.py`.
...
- `auralis/core/config/` — processing configuration (`unified_config.py` → `UnifiedConfig`, plus `preset_profiles.py`, `genre_profiles.py`, `settings.py`, `factory.py`). Legacy dataclasses (`LimiterConfig` etc.) still live in `auralis/core/config.py` — a parameter may exist in both; check which one the call path actually reads.
