...
- **App Wiring**: `auralis-web/backend/config/app.py` (create_app), `auralis-web/backend/config/middleware/` (package: CORS wiring in `__init__.py` + RateLimit + SecurityHeaders + NoCache + OriginCheck, one module each), `auralis-web/backend/config/routes.py` (registers all 20 routers), `auralis-web/backend/config/startup/__init__.py` (lifespan; its steps are the package's submodules, #5236), `auralis-web/backend/config/background_workers.py`, `auralis-web/backend/config/limits.py`, `auralis-web/backend/config/origins.py` (loopback origin policy)
- **Routers**: `auralis-web/backend/routers/` — 36 `.py` files: 20 registered routers (albums, artists, artwork, cache_streamlined, enhancement, files, fingerprint_queue, fingerprint_status, health, library, library_scan, metadata, player, playlists, processing_api, settings, similarity, similarity_graph, system, tracks) + shared helpers (`dependencies.py`, `errors.py`, `pagination.py`, `serializers.py`, `similarity_common.py`) + the unregistered `player_*.py` / `processing_*.py` siblings their factories import (#5472). Derive the live list from `auralis-web/backend/config/routes.py`.
- **WebSocket**: `auralis-web/backend/core/audio_stream_controller.py` plus the handler layer in `auralis-web/backend/ws_handlers/` (`connection.py`, `context.py`, `messages.py`, `playback_commands.py`, `playback_control.py`) and `auralis-web/backend/websocket/` (`websocket_protocol.py`, `websocket_security.py`, `outbound_messages.py` — typed broadcast contracts)
- **Chunked Processor**: `auralis-web/backend/core/chunked_processor.py` is a coordinator (#4245) over `chunk_render.py`, `chunk_streaming.py`, `chunk_batch.py`, `chunk_metadata.py`, `chunk_path_cache.py`, `chunk_processor_init.py`, `chunk_content_profile.py` and `chunk_fingerprint_registry.py` — each takes the processor instance and reads/writes its state, so audit them as one surface. Constants come from `auralis-web/backend/core/chunk_boundaries.py` (`CHUNK_DURATION=15.0`, `CHUNK_INTERVAL=10.0`, `OVERLAP_DURATION=5.0`, `CONTEXT_DURATION=5.0`) — never hardcode them. Each chunk is rendered over a 15s window with context and emitted as a **10s non-overlapping segment; there is no boundary crossfade in the emitted path** (#4642). Related: `chunk_cache_manager.py`, `chunk_mastering.py`, `chunk_operations.py` (render-with-context + trim). `chunk_crossfade.py` has no production caller — see Retired Architecture in `_audit-common.md`.
- **Streaming paths**: `auralis-web/backend/core/stream_enhanced.py`, `auralis-web/backend/core/stream_normal.py`, `auralis-web/backend/core/stream_seek.py`, their per-chunk pumps (`stream_enhanced_chunks.py` — which also runs the look-ahead task, `stream_normal_chunks.py`, `stream_seek_chunks.py`), `auralis-web/backend/core/stream_track_resolution.py` (track lookup + path validation), `auralis-web/backend/core/stream_protocol.py`, `auralis-web/backend/core/stream_messages.py`, `auralis-web/backend/core/proactive_buffer.py`. Shared per-chunk helpers extracted from the controller (#4071) live in `auralis-web/backend/core/stream_chunk_ops.py` (used by the enhanced and seek paths only — the normal path reads chunks off disk with no DSP) and `auralis-web/backend/core/stream_fingerprint.py` (fingerprint readiness before/while adaptive mastering streams). Both take the controller instance as a parameter, so they read its mutable state — treat them as part of the controller's surface, not as free functions.
...
- **Processing Engine & workers**: `auralis-web/backend/core/processing_engine.py` (a coordinator after #4250 — lifecycle, execution, config, progress, finalize, cleanup and error mapping live in `job_lifecycle.py`, `job_execution.py`, `job_config.py`, `job_progress.py`, `job_finalize.py`, `job_cleanup.py`, `job_error_mapping.py`), `auralis-web/backend/core/executors.py` (streaming and I/O thread pools installed at lifespan startup, #5086), `auralis-web/backend/core/job_models.py` (`ProcessingJob` / `ProcessingStatus`, extracted in #4250 to break a circular import and re-exported from `processing_engine.py`), `auralis-web/backend/core/processor_pool.py`, `auralis-web/backend/core/processor_factory.py`, `auralis-web/backend/core/job_worker.py`, `auralis-web/backend/core/streamlined_worker.py`, `auralis-web/backend/core/state_manager.py`
...
- [ ] Segment tiling — each chunk is rendered with context and trimmed to a non-overlapping `CHUNK_INTERVAL` segment. Do consecutive segments tile exactly (no gap, no overlap, no repeated samples), and does the emitted total equal the source length? A boundary crossfade reappearing in the emitted path is a regression of #4642, not a fix.
- [ ] First/last chunk — the first chunk has no leading context and the last is short. Are both trimmed without zero-padding or dropping samples?
...
- [ ] Executor pools (#5086) — per-chunk DSP and chunk reads must go through `run_in_stream_executor` (the reserved stream pool in `auralis-web/backend/core/executors.py`); every other `to_thread` site lands on the I/O pool installed as the loop default. A per-chunk call on plain `to_thread` queues behind repository and library-scan work and can trip `CHUNK_PROCESS_TIMEOUT` on queueing delay alone; per-stream setup moved onto the stream pool steals its reservation.
- [ ] Coordinator siblings — `processing_engine.py` delegates to the `job_*.py` modules, which mutate the engine instance directly. Check locking and state transitions there, not only in the coordinator.
...
- [ ] Preset literal — `VALID_PRESETS` / `EnhancementPresetLiteral` in `auralis-web/backend/schemas/enhancement.py` is the single source and holds only `'adaptive'`. `websocket/outbound_messages.py` imports it; `core/proactive_buffer.py` keeps a local `AVAILABLE_PRESETS` mirror. Flag a mirror that disagrees, a route or WS handler that accepts a preset string without checking `VALID_PRESETS`, and a stored legacy `default_preset` (e.g. `'warm'`) that is echoed back instead of degrading to null/`'adaptive'`.

### Dimension 6: Middleware & Configuration

**Check**:
- [ ] CORS — in `auralis-web/backend/config/middleware/` (`__init__.py`, `hosts.py`): is the allow-origins builder properly restricted? Is `allow_credentials=True` combined with wildcard `["*"]` origins (insecure)?
...
- [ ] Middleware ordering — `add_middleware` is LIFO; verify the resulting request-inbound order (CORS → SecurityHeaders → NoCache → RateLimit) matches the documented intent in `setup_middleware()` (`auralis-web/backend/config/middleware/__init__.py`).
...
- [ ] Startup/shutdown events — does the lifespan in the `auralis-web/backend/config/startup/__init__.py` package (its `shutdown` and `rollback` submodules hold teardown; plus `auralis-web/backend/config/background_workers.py`) initialize and tear down background tasks, DB connections, and engine resources symmetrically?
...
**Key files**: `auralis-web/backend/core/stream_seek.py`, `auralis-web/backend/core/seekable_source.py`, `auralis-web/backend/core/stream_seek_chunks.py`, `auralis-web/backend/core/stream_enhanced_chunks.py`, `auralis-web/backend/core/proactive_buffer.py`, `auralis-web/backend/core/stream_chunk_ops.py`, `auralis-web/backend/core/chunk_boundaries.py`
...
- [ ] Look-ahead cancellation — the enhanced pump in `stream_enhanced_chunks.py` starts rendering the next chunk before it is due. When a seek invalidates that work, is the look-ahead task cancelled and its result discarded, or does it land a stale chunk in the stream or the cache? (The old next-track prefetch was dead code, deleted in #3879 — do not report its absence.)
