...
@pytest.mark.phase5c
class TestAPIEndpointDualModeParametrized:
    """Phase 5C.3: Parametrized dual-mode tests for general library endpoints.
    ...
    def test_main_api_tracks_interface(self, mock_data_source):
        """
        Parametrized test: Validate tracks repository interface for both modes.

        General library endpoint must work with both data access patterns.
        """
    ...
    def test_main_api_get_all_returns_tuple(self, mock_data_source):
        """
        Parametrized test: Validate tracks.get_all returns (items, total) for both modes.

        Both patterns must support the same pagination interface.
        """
        mode, source = mock_data_source
    ...
    def test_main_api_get_by_id_interface(self, mock_data_source):
        """
