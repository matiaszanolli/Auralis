...
def test_rapid_sequential_operations(large_library):
    ...
    - No memory leaks
    """
    db, track_ids, _ = large_library

    # 100 rapid operations
    for i in range(100):
        # Mix of operations
        db.tracks.get_all(limit=10, offset=i % 100)
        if i % 10 == 0:
            db.tracks.set_favorite(track_ids[i % len(track_ids)])
        if i % 5 == 0:
            db.tracks.search(f"Track {i:04d}")
...
def test_large_result_set_handling(large_library):
    ...
    tracks, total = db.tracks.get_all(limit=500)
...
def test_many_small_queries(large_library):
    ...
    results = []
    for i in range(200):
        tracks, _ = db.tracks.get_all(limit=1, offset=i)
...
def test_alternating_read_write_stress(large_library):
    ...
    """
    db, track_ids, tmp_path = large_library
    ...
    for i in range(50):
        # Read
        db.tracks.get_all(limit=10, offset=i * 2)

        # Write (toggle favorite)
        db.tracks.set_favorite(track_ids[i % len(track_ids)])

        # Read again
        db.tracks.get_favorites(limit=10)
...
def test_repeated_favorite_toggle_stress(large_library):
    ...
    for track_id in target_ids:
        for i in range(100):
            db.tracks.set_favorite(track_id, i % 2 == 0)  # Alternate True/False

    # All should end up NOT favorited (100 toggles = even, last is False)
    favorites, total = db.tracks.get_favorites(limit=20)
...
def test_mixed_operation_workflow(large_library):
    ...
    for i in range(20):
        # Browse library
        db.tracks.get_all(limit=20, offset=i * 10)
        ...
        # db.tracks.get_recent(limit=5)

    # Final state should be consistent
    tracks, total = db.tracks.get_all(limit=10)
    assert total == 200

    favorites, total = db.tracks.get_favorites(limit=50)
...
def test_cleanup_after_operations(tmp_path):
    ...
    tracks, total = db.tracks.get_all(limit=5)
    assert total == 10
...
def test_multiple_library_instances(tmp_path):
    """
    BOUNDARY: Multiple LibraryDatabase instances.
    ...
    track1 = db1.tracks.add({
        'filepath': str(filepath),
        'title': 'Multi Instance Track',
    })

    # Read with second instance
    tracks, total = db2.tracks.get_all(limit=10)
...
def test_performance_with_many_favorites(large_library):
    ...
    def get_favorites():
        db.tracks.get_favorites(limit=100)
...
def test_empty_search_results_workflow(large_library):
    ...
    tracks, total = db.tracks.search("NONEXISTENT_QUERY_12345")

    assert tracks is not None
    assert isinstance(tracks, list)
    assert len(tracks) == 0
    # Note: only the result list is asserted here, not the total
...
def test_all_tracks_favorited_workflow(large_library):
    ...
    # Get all favorites
    favorites, total = db.tracks.get_favorites(limit=300)
...
def test_rapid_filter_changes(large_library):
    ...
    for filter_str in filters * 10:
        tracks, total = db.tracks.search(filter_str)
        assert tracks is not None
...
def test_state_consistency_after_errors(tmp_path):
    ...
    try:
        db.tracks.delete(999999)  # Non-existent ID
    # narrowed from bare Exception, #5023: tracks.delete() returns False for an
