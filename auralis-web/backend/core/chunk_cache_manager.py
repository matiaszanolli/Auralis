...
class ChunkCacheManager:
    ...
    get_wav_chunk_path() in ChunkedAudioProcessor resolve to the exact same
    on-disk file via WAVEncoder.get_chunk_path() — the split key meant a hit
    recorded by one caller was invisible to the other, leaving the on-disk
    tier write-only from process_chunk()'s side (#4792). get_wav_cache_key
    was removed; both callers now share get_chunk_cache_key.
    ...
    def get_cached_chunk_path(self, cache_key: str) -> Path | None:
        ...
        Args:
            cache_key: Cache key (from get_chunk_cache_key)
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        Returns:
            ...
            - chunk_entries: Number of cached chunk paths (process_chunk() and
              get_wav_chunk_path() share this one key format since #4792 —
              both address the same on-disk WAV, so there's no longer a
              separate "wav_entries" count)
        ...
        Examples:
            ...
            >>> stats
            {'total_entries': 0, 'chunk_entries': 0, 'fingerprint_entries': 0}
        """
