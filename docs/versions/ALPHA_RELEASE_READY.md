...
> **📜 Historical snapshot (v1.0.0-alpha.1, Oct 2025)**: This document is scoped to the first alpha release and is kept for historical reference. Everything below — including the "Node.js 18+ / Python 3.11+" prerequisites — describes that release, **not** the current one. Current requirements are Python 3.14+ and Node 24+ (see [CLAUDE.md](../../CLAUDE.md)); the current source version is the unreleased 1.5.1 recovery milestone (`auralis/version.py`). Changelog references point to [docs/releases/CHANGELOG.md](../releases/CHANGELOG.md).
