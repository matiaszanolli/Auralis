...
| 6 | Two processor caches | `hybrid_processor_singleton._processor_cache` (max 10, core library) vs `ProcessorFactory` (max 32, backend). **These sit at different layers** — `auralis` core cannot import the backend's `ProcessorFactory` (confirmed), so this is layered, not a naive merge. The real duplication was 3 **unused** `process_adaptive/reference/hybrid` wrappers in `processor_factory` duplicating `hybrid_processor`'s public API | ✅ **Done 2026-07-11** — removed the 3 dead `processor_factory` convenience fns. Two caches **kept** (layered/intentional) |
...
| 7 | Two frontend REST clients: `useRestAPI` + `standardizedAPIClient` | — | ✅ **Done 2026-07-12 — the "two competing clients" premise was stale.** A prior migration (#4300) already moved the sole generic singleton consumer onto `useRestAPI`, which is the canonical generic REST hook (6 domain consumers). `StandardizedAPIClient` only survived as the engine behind three *envelope-aware specializations* — and two of the three (`usePaginatedAPI`, standardized `useBatchOperations`) plus `useInitializeAPI` had **zero production consumers**. Removed all the dead surface: the 3 dead hooks, the `BatchAPIClient` class, the `getPaginated` method, `isPaginatedResponse`, and the orphaned pagination/batch types (`PaginationMeta`/`PaginatedResponse`/`BatchItem`/`BatchRequest`/`BatchItemResult`/`BatchResponse`); deleted the obsolete `useStandardizedAPI.test.ts` (only tested `usePaginatedAPI`) and trimmed the batch/pagination cases from `standardizedAPIClient.test.ts`. **Kept** the legitimate React-Query-backed cache telemetry stack (`useCacheStats`/`useCacheHealth` → `CacheAwareAPIClient`) for its 4 cache components. Verified: client test passes, no new type errors, 1 pre-existing `CacheHealthWidget` role assertion fails identically at HEAD. *Optional follow-up (deferred):* relocate the leaked `CacheStats`/`CacheHealth` types out of the client file into `src/types/`, and extract a shared `fetchWithTimeout` helper both clients could sit on (the one genuine remaining duplication) |
...
| # | Item | Note |
|---|------|------|
| 11 | `SpectrumMapper` → fold into `AdaptiveTargetGenerator` | ✅ **Investigated 2026-07-12 — DO NOT fold (reclassified to do-not-collapse).** They are **complementary layers, not duplicates**: `SpectrumMapper` (preset-anchor geometric interpolation, no config/genre/fingerprint/stereo/LUFS) emits a 5-band `ProcessingParameters` dataclass driving gain-staging + dynamics; `AdaptiveTargetGenerator` (genre+fingerprint+preset+user-learning) emits a LUFS/stereo/EQ target **dict** driving psychoacoustic EQ + stereo width. Both run at *different stages* of the same `AdaptiveMode.process` pass. Folding the smaller, untested, legacy-only `SpectrumMapper` into the fingerprint-driven ATG would drag two schemas + two provenance models into one hot-path class and ship its logic untested. The honest move is to retire `SpectrumMapper` *with* legacy `AdaptiveMode` (see #5) — which product decided to **keep** — so `SpectrumMapper` stays. **Fixed 2 incidental bugs found:** `content_analysis_facade.spectrum_mapper` called `SpectrumMapper(config)` (init takes no args → latent `TypeError`; property was dead → removed), and `hybrid_mode.process` discarded a `generate_targets()` result (dead no-op → removed, wiring noted) |
...
- **`SpectrumMapper` vs `AdaptiveTargetGenerator`** (was #11) — **not duplication.** Complementary
  parameter generators feeding *different stages* of `AdaptiveMode.process`: SpectrumMapper's
  5-band `ProcessingParameters` dataclass drives gain-staging/dynamics; ATG's LUFS/stereo/EQ dict
  drives psychoacoustic EQ/stereo. SpectrumMapper retires only *with* legacy AdaptiveMode (kept),
  not by folding into the hot-path fingerprint-driven ATG.
