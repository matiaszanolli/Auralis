...
> **Update (2026-09-15, #5442)**: the `numpy`-rs 0.29 ABI error above **no longer reproduces**. Re-tested against NumPy 2.4.6 (the version now pinned everywhere) on CPython 3.14.0, a pyo3/numpy 0.29 build passes all 11 array-accepting entry points, with outputs bit-identical to the shipped 0.23 build. The bump is tracked in #5485; see `vendor/auralis-dsp/UPGRADE_PLAN.md`.
